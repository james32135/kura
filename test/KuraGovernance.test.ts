...
describe("KuraGovernance", function () {
  ...
  it("members can cast encrypted votes (yes and no)", async function () {
    ...
    const yesVote = await cofheClient.encrypt_bool(true);
    const noVote = await cofheClient.encrypt_bool(false);
  ...
  it("member cannot vote twice on same proposal", async function () {
    await governance.connect(member1).createProposal(CIRCLE_ID, "Test proposal", DURATION, QUORUM);
    const vote = await cofheClient.encrypt_bool(true);
  ...
  it("non-voter gets encrypted absence proof via FHE.not", async function () {
    ...
    const proof = await governance.connect(member3).getEncVoteAbsenceProof(1, member3.address);
  ...
  it("voter gets absence proof = false via FHE.not", async function () {
    ...
    const proof = await governance.connect(member1).getEncVoteAbsenceProof(1, member1.address);
  ...
  it("verifyMajority returns encrypted bool based on yes count", async function () {
    ...
    const yesVote = await cofheClient.encrypt_bool(true);
    await governance.connect(member1).submitVote(1, yesVote);
    await governance.connect(member2).submitVote(1, yesVote);

    // yes count = 2, threshold = 1 → FHE.gte(2, 1) = true
    const result = await governance.connect(owner).verifyMajority(1, 1n);
    await hre.cofhe.mocks.expectPlaintext(result, 1n);
  });

  // ─── FHE.verifyDecryptResultBatch: closeVoteBatch ────────────────────────

  it("admin can close vote with batch verification and emit final result", async function () {
    await governance.connect(member1).createProposal(CIRCLE_ID, "Test proposal", DURATION, QUORUM);
    ...
    // Get batch decryption sigs from mock CoFHE
    const yesSig = await hre.cofhe.mocks.getDecryptSig(encYes, 2n);
    const totalSig = await hre.cofhe.mocks.getDecryptSig(encTotal, 3n);

    await governance.connect(owner).closeVoteBatch(1, 2n, 3n, yesSig, totalSig);
  ...
  it("non-admin cannot close vote", async function () {
    await governance.connect(member1).createProposal(CIRCLE_ID, "Test proposal", DURATION, QUORUM);
    await expect(
      governance.connect(outsider).closeVoteBatch(1, 0n, 0n, "0x", "0x")
